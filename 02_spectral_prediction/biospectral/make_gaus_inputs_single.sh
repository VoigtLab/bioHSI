...
if [ "$XTB" = true ]; then
	mkdir "$OUTPUT_DIR/xtb_out" #output directory for xtb files
	FILE="$INPUT_DIR/$NAME.$IN_EXT" #file with 3D molecular coordinates

	if [ "$OVERWRITE" = false ] && [ -f  "$OUTPUT_DIR/$NAME.com" ]; then
		echo "File $OUTPUT_DIR/$NAME.com already exists. Skipping."
	else		
		XTBOUT="$OUTPUT_DIR/xtb_out/$NAME" #prefix for xtb output
		input="$OUTPUT_DIR/${NAME}_in.xyz" #input file with 3D molecular coordinates in .xyz format
		XTBOUTFILE="$XTBOUT.gau" #output file with 3D molecular cooridnates in appropriate format for Gaussian (including charge and multiplicity)
